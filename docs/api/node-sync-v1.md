...
array, which is valid for `HEARTBEAT` and `RECEIPT_SUBMITTED`.
...
| `receipt_id`                  | payment proof | UUID generated by Lite                              |
| `receipt_date`                | payment proof | ISO date                                            |
| `proof_content_type`          | payment proof | `image/jpeg`, `image/png`, or `image/webp`          |
| `proof_image_base64`          | payment proof | matching image bytes, Base64 encoded; at most 3 MiB |
| `utr_number`                  | no            | string, at most 80 characters                       |
...
| `RECEIPT_SUBMITTED`   | Stores a Lite payment-proof image and optional UTR as `PENDING` for Master approval or denial; `items` must be empty                                                         |
...
`TRANSFER_RECEIVED` requires `transfer_id`. `RECEIPT_SUBMITTED` requires its
receipt fields above. All other non-heartbeat events require at least one item.
...
- `RECEIPT_REVIEWED`: originating Lite records the Master `APPROVED` or `DENIED`
  decision. A denied receipt always carries rejection remarks.
