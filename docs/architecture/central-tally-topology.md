...
The central Tally worker processes pending supported vouchers sequentially. A Tally failure leaves the voucher pending or failed on Master for review. Acceptance of an event through `/api/v1/events` is not a Tally success acknowledgement. Tally runs at Master, so no XML is sent back to Lite for local Tally import.
